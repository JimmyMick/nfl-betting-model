...
| **Make picks** | Lock in your winners each week. |
| **Season tracker** | How the model's doing vs Vegas. |
| **Weekly preview** | The model's read on this week (+ the AI's picks). |
| **📈 Paper play** | A fake-money experiment betting one game a week. |
| **📖 Guide** | You're here. |

## ✅ Making picks (do this every week)
1. Open **Make picks**.
...
Yes, we *just* said betting the model's edges loses money — and betting **all** of
them does. But we went digging and found one weird exception: if each week you
bet **only the single biggest disagreement** on the board (and take the *model's*
side, usually an underdog), the last 10 years of history would've actually
**made money** (+23%). Could be a real edge. Could be dumb luck that the past
happened to reward. There's no way to know from old data — the model was built
looking at those same years, so of course it looks good there.
...
- **After grading:** open the **"🤖 why it picked"** drawer on the leaderboard to
  see its logic next to the ✓/✗.
