"""Throwaway: multi-season walk-forward of the weather feature.

For each test season, compares (with vs without the game-time weather feature)
on calibration metrics, on two base feature sets:
  - ISOLATION: Elo+EPA only (is the signal real?)
  - FULL: + QB + Starters (does it survive next to the full validated player set
    the live predict.py uses?)
Trains on every prior season (expanding window). Mirrors validate_avail.py.
"""
...
def compare(name, base_kwargs):
    ...
    df_a, cols_a = build_features(games, weather_table=weather, **base_kwargs)
    print(f"\n=== {name} ({len(cols_b)}→{len(cols_a)} feats, {CALIBRATE}) ===")
    print(f"  {'season':>6}  {'logloss base→+wx':>22}  {'brier base→+wx':>20}  "
          f"{'auc base→+wx':>18}")
